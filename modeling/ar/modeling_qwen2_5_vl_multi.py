...
from transformers.generation import GenerationMixin, GenerateDecoderOnlyOutput, LogitsProcessorList, StoppingCriteriaList, GenerationConfig, GenerateEncoderDecoderOutput
...
@dataclass
class GenerateDecoderOnlyAll2AllOutput(GenerateDecoderOnlyOutput):
    output_image_embeddings: Optional[torch.Tensor] = None
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def prepare_prefilled_image_embeds(self, cur_image_tokens, num_img_tokens, is_sampling_img, **model_kwargs):
        ...
        # comment this
        # image_idx = torch.tensor([self.config.image_token_id]).to(self.device).long().unsqueeze(0)
        # inputs_embeds = self.model.embed_tokens(image_idx)
